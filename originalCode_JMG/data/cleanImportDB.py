    ...
    results = db.run(
        "MATCH (n1) WHERE $source_id IN n1.D" + str(depth) + "_id "
                                                             "MATCH (n2) WHERE $target_id IN n2.D" + str(depth) + "_id "
                                                                                                                  "CREATE (n1)-[:LINKS_TO_D" + str(
            depth) + " {count: $count}]->(n2) "
...
def import_monde_diplo_files(path_to_medias_francais, path_to_relations_medias_francais):
    ...
    for index, row in medias_df.iterrows():
        i = 1 + 1
        node_match = graph.nodes.match(entity_name=row["nom"]).first()
        if node_match is  None:
            try:
                node_match = Node('Entity', entity_name=row["nom"])
                node_match.__primarylabel__ = 'Entity'
                node_match.__primarykey__ = 'entity_name'
                graph.merge(node_match)
            except:
                print("could not import ", row["nom"])
                node_match = None
        if node_match is not None:
            for key in row.keys():
                node_match["Diplo_" + key] = row[key]
            graph.push(node_match)
        if i % 100 == 0:
            print(".", end=" ")
        if i % 1000 == 0:
            print(i, "/", totnbnodes)
    ...
    for index, row in relations_df.iterrows():
        ...
        results = db.run(
            ...
            , {"origine": row['origine'], "cible": row['cible'], "v": row['valeur'], "s": row['source'],
               "dp": row['datePublication'], "dc": row['dateConsultation']}
        )
        if j % 100 == 0:
            print(".", end=" ")
        if j % 1000 == 0:
...
def importACPM_SiteGP(ACPM_siteGrandsPublics_path):
    ...
    ACPM_key = {"L'Aisnenouvelle.fr": "aisnenouvelle.fr", "L'Equipe.fr": "lequipe.fr",
                ...
                "Diabétologie-pratique.com": "diabetologie-pratique.com",
                "Gynécologie-pratique.com": "gynecologie-pratique.com"}
    ...
    for index, row in df.iterrows():
        ...
        if match is  not None:
            ...
            for key in row.keys():
                if match["ACPM_SiteGP_" + key] is  None:
...
def importACPM_SitePro(ACPM_sitePro_path):
    ...
    for index, row in df.iterrows():
        ...
        if match is  not None:
            ...
            for key in row.keys():
                if match["ACPM_SitePro_" + key] is  None:
